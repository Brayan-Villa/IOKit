/*
<title>iokit/IOKit/IOService.h Source</title>
</head><body>
<h1>Source to iokit/IOKit/IOService.h</h1>
 ...
 *
 * @<a href="/cdn-cgi/l/email-protection" class="__cf_email__" data-cfemail="5c1d0c0c10190310151f19120f190314191d18190e030f081d0e081c">[email&#160;protected]</a>
 * 
 ...
 * under the License.
 * 
 * @<a href="/cdn-cgi/l/email-protection" class="__cf_email__" data-cfemail="c584959589809a898c86808b96809a8d80848180979a808b8185">[email&#160;protected]</a>
 */
/*
...
/*! @function probe
    ...
    @param score Pointer to the current driver's probe score, which is used to order multiple matching drivers in the same match category. It defaults to the value of the Probe Score property in the drivers property table, or kIODefaultProbeScore if none is specified. The probe method may alter the score to affect start order.
...
/*! @function matchPropertyTable
    ...
    @discussion All matching on an IOService will call this method to allow a family writer to implement matching in addition to the generic methods provided by IOService. The implementor should examine the matching dictionary passed to see if it contains properties the family understands for matching, and use them to match with the IOService if so. Note that since matching is also carried out by other parts of IOKit, the matching dictionary may contain properties the family does not understand - these should not be considered matching failures.
...
public:
    ...
    virtual void doServiceMatch( void );
...
/*! @function registerControllingDriver
...
/*! @function setParent
        ...
        @param currentState
        These flags describe the character of power in the power domain.  If the
        policy-maker has a controlling driver, the policy-maker asks the driver,
        given this power domain state, what state it would be in, and then it
        tells the driver to assume that state. */
...
/*! @function overrideOnPriv
    ...
    calls overrideOnPriv in its superclass to enable this override.  When the override
    is on, the superclass keeps the device in the state desired by the policy-maker
    (requested via changeStateToPriv), regardless of the children's or driver's desire.
...
private:
    IOReturn enqueuePowerChange ( unsigned long, unsigned long, unsigned long );
    IOReturn notifyAll ( bool is_prechange );
    bool notifyObject ( IOPMinformee * nextObject, bool is_prechange, bool is_child );
    void our_prechange_1 ( void );
    void our_prechange_2 ( void );
    void our_prechange_3 ( void );
    void our_prechange_4 ( void );
    IOReturn parent_prechange_down_1 ( void );
    IOReturn parent_prechange_down_2 ( void );
    void parent_prechange_down_3 ( void );
    void parent_prechange_down_4 ( void );
    void parent_prechange_down_5 ( void );
    void parent_prechange_up_1 ( void );
    IOReturn parent_postchange_down_1 ( void );
    void parent_postchange_down_2 ( void );
    IOReturn parent_postchange_up_1 ( void );
    IOReturn parent_postchange_up_2 ( void );
    IOReturn parent_postchange_up_3 ( void );
    void parent_postchange_up_4 ( void );
    void parent_postchange_up_5 ( void );
    void parent_postchange_up_6 ( void );
    void all_done ( void );
    void all_acked ( void );
    ...
    IOReturn add_to_active_change ( IOPMinformee * newObject, bool is_child );
